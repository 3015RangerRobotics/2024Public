...
public class SwerveSetpointGenerator {
  ...
  public SwerveSetpointGenerator(final SwerveDriveKinematics kinematics) {
    ...
    this.numModules = kinematics.toSwerveModuleStates(new ChassisSpeeds()).length;
  }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    if (Util.epsilonEquals(desiredState, new ChassisSpeeds())) {
      need_to_steer = false;
    ...
    if (all_modules_should_flip
        && !Util.epsilonEquals(prevSetpoint.chassisSpeeds, new ChassisSpeeds())
        && !Util.epsilonEquals(desiredState, new ChassisSpeeds())) {
    ...
    final double max_theta_step = dt * limits.maxSteerVelocity();
    for (int i = 0; i < numModules; ++i) {
      if (!need_to_steer) {
