...
class _CustomDropdownMenuState<T> extends State<CustomDropdownMenu<T>> {
  ...
  void initState() {
    ...
    _menuHasEnabledItem = filteredEntries
        .any((CustomDropdownMenuEntry<T> entry) => entry.enabled);
  ...
  List<CustomDropdownMenuEntry<T>> filter(
    ...
    return entries
        .where((CustomDropdownMenuEntry<T> entry) =>
            entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  int? search(List<CustomDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((CustomDropdownMenuEntry<T> entry) =>
        entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<CustomDropdownMenuEntry<T>> filteredEntries,
  ...
  Widget build(BuildContext context) {
    ...
    final DropdownMenuThemeData defaults =
        _CustomDropdownMenuDefaultsM3(context);
      ...
      child: Actions(
        ...
        child: MenuAnchor(
          ...
          builder:
            ...
            return _CustomDropdownMenuBody(
              width: widget.width,
              children: <Widget>[
                Focus(
                  onFocusChange: (value) {
                    if (true && !_controller.isOpen) {
                      handlePressed(_controller);
                    }
                  },
                  child: TextField(
                      key: _anchorKey,
...
class _CustomDropdownMenuBody extends MultiChildRenderObjectWidget {
  const _CustomDropdownMenuBody({
    super.children,
    this.width,
  });
  ...
  @override
  _RenderCustomDropdownMenuBody createRenderObject(BuildContext context) {
    return _RenderCustomDropdownMenuBody(
      width: width,
    );
  }
  ...
  void updateRenderObject(
      BuildContext context, _RenderCustomDropdownMenuBody renderObject) {
    renderObject.width = width;
  }
}

class _CustomDropdownMenuBodyParentData
    extends ContainerBoxParentData<RenderBox> {}

class _RenderCustomDropdownMenuBody extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox,
            _CustomDropdownMenuBodyParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox,
            _CustomDropdownMenuBodyParentData> {
  _RenderCustomDropdownMenuBody({
    double? width,
  }) : _width = width;
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! _CustomDropdownMenuBodyParentData) {
      child.parentData = _CustomDropdownMenuBodyParentData();
    }
  }
  ...
  void performLayout() {
    ...
    while (child != null) {
      if (child == firstChild) {
        ...
        final _CustomDropdownMenuBodyParentData childParentData =
            child.parentData! as _CustomDropdownMenuBodyParentData;
...
class _CustomDropdownMenuDefaultsM3 extends DropdownMenuThemeData {
  _CustomDropdownMenuDefaultsM3(this.context);
